...
// 注册推广位管理路由
func InitAdRoutes(r *gin.RouterGroup, authMiddleware *jwt.GinJWTMiddleware) gin.IRoutes {
	adController := controller.NewAdController()
	router := r.Group("/ad")
	// 开启jwt认证中间件
	...
	{
		router.GET("/:id", adController.GetAdInfo)
