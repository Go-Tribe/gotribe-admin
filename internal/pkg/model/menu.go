...
type Menu struct {
	...
	Sort       uint    `gorm:"type:integer;default:999;comment:菜单顺序(1-999)" json:"sort"`
	Status     uint    `gorm:"type:smallint;default:1;comment:菜单状态(正常/禁用, 默认正常)" json:"status"`
	Hidden     uint    `gorm:"type:smallint;default:2;comment:菜单在侧边栏隐藏(1隐藏，2显示)" json:"hidden"`
	NoCache    uint    `gorm:"type:smallint;default:2;comment:菜单是否被 <keep-alive> 缓存(1不缓存，2缓存)" json:"noCache"`
	AlwaysShow uint    `gorm:"type:smallint;default:2;comment:忽略之前定义的规则，一直显示根路由(1忽略，2不忽略)" json:"alwaysShow"`
	Breadcrumb uint    `gorm:"type:smallint;default:1;comment:面包屑可见性(可见/隐藏, 默认可见)" json:"breadcrumb"`
