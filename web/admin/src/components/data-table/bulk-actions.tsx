...
export function DataTableBulkActions<TData>({
  ...
  useEffect(() => {
    if (selectedCount > 0) {
      const message = `${selectedCount} ${entityName}${selectedCount > 1 ? 's' : ''} ${t('components.dataTable.bulkActions.selected')}. ${t('components.dataTable.bulkActions.bulkActionsFor')} toolbar is available.`
  ...
  return (
    <>
      ...
      <div
        ref={toolbarRef}
        role='toolbar'
        aria-label={`${t('components.dataTable.bulkActions.bulkActionsFor')} ${selectedCount} ${t('components.dataTable.bulkActions.selected')} ${entityName}${selectedCount > 1 ? 's' : ''}`}
      ...
      >
        ...
        >
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                title={t('components.dataTable.bulkActions.clearSelectionEscape')}
              >
                <X />
                <span className='sr-only'>{t('components.dataTable.bulkActions.clearSelection')}</span>
              </Button>
            </TooltipTrigger>
            <TooltipContent>
              <p>{t('components.dataTable.bulkActions.clearSelectionEscape')}</p>
            </TooltipContent>
          </Tooltip>
          ...
          >
            ...
            {t('components.dataTable.bulkActions.selected')}
          </div>
